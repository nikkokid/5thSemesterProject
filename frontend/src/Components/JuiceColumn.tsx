...
import {updateJuiceById, deleteJuiceById, type Juice, type CreateJuice } from "../Services/Juice/JuiceService";
import AddJuiceDialogContent from "./AddJuiceDialogContent";
    ...
    <div>
      ...
      {juices.length === 0 ? (
        <div className="border rounded-lg p-6 text-center">
          ...
          <button onClick={handleAddJuice} className="bg-green-600! text-white">
            Tilføj {title}
          </button>
        </div>
      ) : (
        <div>
          {juices.map((juice) => (
            <div
              className="grid grid-cols-1 sm:grid-cols-2 gap-5 border rounded-lg p-4 sm:p-6"
              key={juice.id}
            >
              <div className="sm:col-span-2 border bg-gray-100 rounded-lg p-2 mb-4">
                ...
                {editingJuiceId === juice.id ? (
                  <div className="border rounded p-4 mb-3 bg-white">
                    <h3 className="text-lg mb-4">Rediger Most {juice.id}</h3>
                    ...
                    <form action={(formData) => handleEditJuice(juice.id, formData)} className="flex flex-col">
                      <p>
                        ...
                        <input
                          type="number"
                          name="editedJuiceVolume"
                          defaultValue={juice.volume}
                      ...
                      <p>
                        ...
                        <input
                          type="date"
                          name="editedJuiceDate"
                          defaultValue={juice.pressedDate.split("T")[0]}
                          className="border p-1 mb-2 w-full"
                        />
                      </p>
                  ...
                  <div>
                    <p>Volumen: {juice.volume} L</p>
                    <p>Presset Dato: {juice.pressedDate.split("T")[0]}</p>
                  </div>
                )}
              </div>
